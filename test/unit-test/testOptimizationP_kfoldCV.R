# In this file, there are some unit tests that check the right execution of the kFoldCV when setting p
...
reg.lm <- snn(medv ~ ., BostonHousing, subset = s, regularization = FALSE,
              ...
              p.control = list(method = 'CV'), y = TRUE)
gSimils <- as.matrix(reg.lm$learn.data[, - ncol(reg.lm$learn.data)])
gY <- reg.lm$y
...
reg.lm <- snn(Type ~ ., wine, regularization = FALSE,
              clust.control = list(clust.method = "PAM", nclust.method = "C"), y = TRUE)
gSimils <- as.matrix(reg.lm$learn.data[, - ncol(reg.lm$learn.data)])
#colnames(simils) <- paste('X', colnames(simils), sep = "")
gY <- reg.lm$y
