# Source of Experiment 10 (Tests the number of rows used to train each SNN of an Ensemble).
...
runExperiment10 <- function(datasets, nRuns = 1, classification = FALSE) {
  ...
  for (ds in datasets) {
    ...
    for (nrow.prop in seq(0, 1, 0.1)) {
      ...
      save(shortResult, fullResults, file = "experiments/Exp10/Exp10TMPData.Rdata")
    }
    save(shortResult, fullResults, file = "experiments/Exp10/Exp10TMPData.Rdata")
  }
